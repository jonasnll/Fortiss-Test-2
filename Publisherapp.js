...
const gatewayClient = new ibmiotf.IotfGateway({                  //neuer Client als IoT device mit mienem Acc und den nötigen Daten erzeugen
    ...
    type: "FlexibleManufacturingGateway",
    "auth-method": "token",
            ...
            if(error === null) {
                ...
                if (alreadyUploaded.indexOf(filePath) === -1) {            // liegt im array schon der/ein dateiname (filepath), wenn ja dann dann index >= 0       EIN Index von etwas was nicht im Array steht ist immer -1
                    ...
                    const readHandler = (error, data) => {
                        ...
                        let obj = JSON.parse(content);                         // Data-String zu einem JSO geparsed
                        ...
                        for( i=0; i<obj.Input.length; i++ ){
                            ...
                            for(j=0; j<obj.Input[i].Values.length; j++){
                        ...
                        for( i=0; i<obj.Output.length; i++ ){
                            ...
                            for(j=0; j<obj.Output[i].Values.length; j++){
...
gatewayClient.on("connect", connectionHandler);                             //Wenn connected dann connection handler                         //Was bedeutet deviceClient.on ? >>> on = wartet auf event >>> connect, dann für den code aus
